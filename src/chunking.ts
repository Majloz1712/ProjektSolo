...
export type ChunkingInput = {
  text: string;
  url?: string;
  previous?: { chunks: PreviousChunk[] } | null;
...
export type ChunkingResult = {
  version: "1.0";
  model: { provider: "ollama"; name: string; options: Record<string, unknown> };
  source: { url: string | null; input_chars: number; input_units: number };
...
type UnitType = "heading" | "paragraph" | "list" | "line";
...
function guessChunkCount(charCount: number): number {
  if (charCount < 8_000) {
    return 4;
  }
...
function buildUnits(text: string): Unit[] {
  ...
  let bufferType: UnitType = "paragraph";
  let offset = 0;
...
function createDescriptor(unit: Unit): UnitDescriptor {
  const maxHead = 320;
  const maxTail = 140;
...
async function chunkWithAnchors(
  units: Unit[],
...
function compactDescriptors(
  ...
  maxPromptChars: number,
): { descriptors: UnitDescriptor[]; mapping: number[] } {
  ...
  const compacted: UnitDescriptor[] = [];
...
function buildChunkPrompt(descriptors: UnitDescriptor[], targetCount: number) {
  return {
    system:
...
function buildAnchorPrompt(previous: PreviousChunk[], targetCount: number) {
...
function buildAssignPrompt(
  anchors: { id: number; title: string; keywords: string[] }[],
  descriptors: UnitDescriptor[],
  minChunkId: number,
  targetCount: number,
) {
...
async function callOllamaJson(
  prompt: { system: string; user: Record<string, unknown> },
  options: Required<ChunkingOptions>,
  label: string,
) {
  const payload = {
...
function parseBoundaries(
  ...
  if (
    ...
    !Array.isArray((response as { boundaries?: unknown }).boundaries)
  ) {
    throw new Error("Invalid boundaries response.");
  }
  const boundaries = (response as { boundaries: number[] }).boundaries
...
function parseAnchors(
  response: unknown,
  targetCount: number,
): { id: number; title: string; keywords: string[] }[] {
  if (
    !response ||
    typeof response !== "object" ||
    !Array.isArray((response as { anchors?: unknown }).anchors)
  ) {
    throw new Error("Invalid anchors response.");
  }
  const anchors = (response as { anchors: { id: number; title: string; keywords: string[] }[] })
    .anchors;
...
function parseAssignments(
  ...
  if (
    !response ||
    typeof response !== "object" ||
    !Array.isArray((response as { assignments?: unknown }).assignments)
  ) {
    throw new Error("Invalid assignments response.");
  }
  const assignments = (response as { assignments: number[] }).assignments
    .map((value) => Math.floor(Number(value)))
    .filter((value) => Number.isFinite(value));
...
function rangesFromAssignments(assignments: number[]): Range[] {
  const ranges: Range[] = [];
  let start = 0;
...
function heuristicRanges(units: Unit[]): Range[] {
  const ranges: Range[] = [];
  let start = 0;
...
function buildChunks(units: Unit[], ranges: Range[]): ChunkResult[] {
...
function buildUnitMap(ranges: Range[], unitCount: number): number[] {
...
function deriveTitle(units: Unit[], fallback: string): string {
...
function createExcerpt(text: string, headSize: number, tailSize: number): string {
